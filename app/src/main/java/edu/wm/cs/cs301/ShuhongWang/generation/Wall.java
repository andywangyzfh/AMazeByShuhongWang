...
//    private Color col;
    ...
    public Wall(final int startX, final int startY, final int extensionX, final int extensionY,
            final int distance, final int cc, MazePanel panel) {
...
//            break;
//        }
    	// if the maze is not in dry-run mode, initialize the color of walls
    	if (mazePanel != null) {
			this.rgb = mazePanel.getWallColor(distance, cc, this.getExtensionX());
    	}
    }
...
//                || (col.getRGB() != o.col.getRGB()) 
                || (mazePanel.getColor() != o.mazePanel.getColor())) {
            return false;
        }
...
//    	mazePanel.setColor(rgb);
    ...
    public Wall[] calculatePartitioning(Wall splitter, int colchange) {
    	...
    	result[0] = new Wall(getStartPositionX(), getStartPositionY(), 
    			...
    			getDistance(), colchange, mazePanel);
