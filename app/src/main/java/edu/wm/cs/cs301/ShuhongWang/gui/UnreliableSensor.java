...
/**
 * This class provides an unreliable sensor for the robot, which could fail and be repaired.
 * 
 * Responsibility: detect distance to obstacles, handle working and failing states, repair it self when failed.
 * 
 * Collaborators: Maze, UnreliableRobot
 * 
 * @author Shuhong Wang
 *
 */
public class UnreliableSensor implements DistanceSensor, Runnable {
	...
	public void stopFailureAndRepairProcess() {
		...
		if (null != sensorThread) {
			// if not null, turn off thread.
			try {
				reliable = false;
				sensorThread.join();
				working = true;
			} catch (Exception e) { 
				System.out.println("UnreliableSensor.stopFailureAndRepairProcess: join synchronization with builder thread lead to an exception") ;
			}
		}
		else {
			System.out.println("UnreliableSensor.stopFailureAndRepairProcess: no thread to wait for");
		}

	}

	/**
	 * The run method of the thread. When the failing and repairing process start, the sensor
	 * is set to fail. After the repairing time, the sensor is set to working. After the
	 * fail time, the robot is set to work again.
	 */
	@Override
	public void run() {
		try {
			// When the process is on, repeating on the process.
			while (reliable) {
				working = false;
				Thread.sleep(repairTime*1000);
				working = true;
				Thread.sleep(failTime*1000);
			}
		} catch (InterruptedException ex) {
			// if interrupted, print error message
			System.out.println("Sensor thread interrupted.");
		}
	}
	
	/**
	 * Get the working status of the sensor.
	 * 
	 * @return working the working status of the sensor.
	 */
	...
	/**
	 * Get whether the sensor is currently reliable or not.
	 * @return reliable true if the sensor is not in a failure repair process, false otherwise.
	 */
